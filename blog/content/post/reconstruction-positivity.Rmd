...
```{proof}
For each, $m$, fine the set $A_m(z) = \{x : \hat Z(x) \neq z\}$.
Because $\hat Z(X)$ is consistent, $P(X \in A(z) \mid Z=z) \rightarrow 0$ as $m$ grows large for each $z$ in the support of $Z$.
Because $Z$ is not degenerate, $P(Z = z) > 0$ for more than one value $z$.
Thus, as $m$ grows large and $\hat Z \rightarrow Z$, $P(X \in A_m(z)) \rightarrow P(Z \neq z) > 0$ for all $z$ in the support of $Z$.
