...
**If $Z$ were observed**, the following assumptions are sufficient to answer the causal query nonparametrically:
  ...
  1. **Positivity:** $P(X \in A \mid Z) > 0 \quad \text{for each } P(A) > 0$, where $A$ is any set in the sample space of $X$.
...
$$
Under the postivity assumption, all terms of the rightmost expression have observable implications, and are therefore estimable.
...
```{proof}
Define the set $A(Z) = \{x : \hat Z(x) \neq Z\}$.
Because $\hat Z(X)$ is consistent, $P(X \in A(Z) \mid Z) \rightarrow 0$ as $m$ grows large.
Because $Z$ is not degenerate, $A(Z)$ has non-zero probability for all values in the support of $Z$.
Thus, $P(X \in A(Z)) > 0$, and the positivity assumption is violated in the limit.
```
...
Sensitivity analysis my be a more fruitful approach if one suspects that this is a problem in a particular study.
