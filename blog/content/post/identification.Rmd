...
The goal of this notebook is to highlight a critical gap between predictive and causal inference. In particular, we consider the problem of **identification**, which is a central issue in causal inference that does not arise in predictive inference. The causal parameters in a problem are **identified** if the set of causal parameters that generate the observable data are unique.
...
This is the simplest possible structure for a latent confounding variable.

Here, we show a counter-example to the claim that one can, in general, construct a consistent estimate of the structural equation of that generates $Y$ given $X$ and $Z$ in this setting.
...
### Constructing the Ignorance Region
...
### Ignorance Region as $m$ Grows Large
...
Latent variable models commonly used to do this exploration.
